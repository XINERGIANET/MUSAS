...
@section('header')
<h1>Almacenamiento de Productos Industrializados</h1>
<p>Lista de Registros de Productos Industrializados</p>
@endsection
                        ...
                        <div class="row mb-6">
                            ...
                            <div class="col-md-4">
                                <div class="input-group">
                                    <input type="text" class="form-control" id="textoBusqueda" placeholder="Buscar Nombre Productos Industrializados...">
                                </div>
                            </div>
                            <div class="col-md-8 d-flex justify-content align-items-center">
                                <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addModal">
                                    <i class="bi bi-plus-circle"></i> Nuevo Producto Industrializado
                                </button>
                                ...
                                <a href="{{ route('stock.stockInicial', ['categoryId' => 2]) }}" class="btn btn-warning">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @endif
                                            @if($storage->id)
                                            <form action="{{ route('storage2.destroy', $storage->id) }}" method="POST" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
...
<div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addModalLabel">Nuevo Producto Industrializado</h5>
...
<script>
    $(document).ready(function() {
        $('#editModal').on('show.bs.modal', function(event) {
            ...
            if (isVirtual) {
                form.attr('action', `{{ url('storage2') }}/0`);
            } else {
                form.attr('action', `{{ url('storage2') }}/${id}`);
            }
        ...
        fetch('{{ route("storage2.store") }}', {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    ToastMessage.fire({
                        icon: 'success',
                        text: data.message || 'Producto Industrializado creado exitosamente'
                    }).then(() => {
                        // Limpiar formulario
                    ...
                    // Error del servidor
                    ToastError.fire({
                        text: data.message || 'Error al crear el producto Industrializado'
                    });
                }
            })
            .catch(error => {
