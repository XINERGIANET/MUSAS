...
<head>
    <meta charset="utf-8">
    <title>Reporte de Compras - {{ $product->nombre }}</title>
    <style>
        body {
...
<body>
    <div class="header">
        <h1>REPORTE DE {{ strtoupper($tipo === 'egreso' ? 'EGRESOS' : 'COMPRAS') }} POR PRODUCTO</h1>
    ...
    @if(isset($productData['message']))
        <div class="no-data">
            ...
            <p>No hay {{ $tipo === 'egreso' ? 'egresos' : 'compras' }} registradas para este producto en el período seleccionado.</p>
        </div>
    @elseif(empty($productData['details']))
        <div class="no-data">
            <h3>Sin movimientos registrados</h3>
            <p>No se encontraron {{ $tipo === 'egreso' ? 'egresos' : 'compras' }} para este producto con los filtros aplicados.</p>
        </div>
    @else
    ...
    <div class="footer">
        Reporte generado automáticamente por el sistema de gestión de {{ $tipo === 'egreso' ? 'egresos' : 'compras' }}
    </div>
</body>
</html>