...
@section('header')
<h1>Almacenamiento de Insumos</h1>
<p>Lista de Registros de Almacenamiento de Insumos</p>
@endsection
                        ...
                        <div class="row mb-6">
                            ...
                            <div class="col-md-4">
                                <div class="input-group">
                                    <input type="text" class="form-control" id="textoBusqueda" placeholder="Buscar Nombre Insumos...">
                                </div>
                            </div>
                            <!-- Botón a la derecha -->
                            <div class="col-md-8 d-flex justify-content align-items-center">
                                ...
                                <a href="{{ route('stock.stockInicial', ['categoryId' => 4]) }}" class="btn btn-warning">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @if($storage->id)
                                            <form action="{{ route('storageInsumo.destroy', $storage->id) }}" method="POST" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
...
<div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addModalLabel">Nuevo Insumo</h5>
...
<script>
    $(document).ready(function() {
        $('#editModal').on('show.bs.modal', function(event) {
            ...
            if (isVirtual) {
                form.attr('action', `{{ url('storageInsumo') }}/0`);
            } else {
                form.attr('action', `{{ url('storageInsumo') }}/${id}`);
            }
    ...
    function saveProduct() {
        ...
        const csrfToken = $('#addProductForm input[name="_token"]').val();

        // Preparar datos
        const formData = new FormData();
        formData.append('nombre', productName);
        formData.append('category_id', categoryId);
        formData.append('_token', csrfToken); // CORRECCIÓN: agregar '_token' como nombre del campo

        // Realizar petición AJAX
        fetch('{{ route("storageInsumo.store") }}', {
                method: 'POST',
                body: formData,
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Opcional: recargar la tabla o lista de productos
                    // location.reload(); // Descomenta si necesitas recargar la página
        ...
        fetch('{{ route("storageInsumo.store") }}', {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    ToastMessage.fire({
                        icon: 'success',
                        text: data.message || 'Insumo creado exitosamente'
                    }).then(() => {
                        // Limpiar formulario
                        document.getElementById('nombre').value = '';
...
<script>
    $(document).ready(function() {
        $('#textoBusqueda').on('input', function() {
            ...
            $.ajax({
                url: '{{ route("storageInsumo.search") }}',
                type: 'GET',
                data: {
                    query: valor
                },
