...
@if(auth()->user()->hasRole('admin'))
    <ul class="nav justify-content-center">
        <li class="nav-item" style="margin: 0 10px 5px 10px;">
            <a class="nav-link btn btn-primary active" href="{{ route('payment.cashClose') }}">Registro</a>
        </li>
        <li class="nav-item" style="margin: 0 10px 5px 10px;">
            <a class="nav-link btn btn-secondary" href="{{ route('cashClose.historico') }}">Histórico</a>
        </li>
    </ul>
@endif
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" name="turno" id="turno-select">
                                        <option value="0" {{ $turno == 0 ? 'selected' : '' }}>Mañana</option>
                                        <option value="1" {{ $turno == 1 ? 'selected' : '' }}>Tarde</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                    ...
                    <form action="{{ route('payment.storeCashClose') }}" id="store-cash-close-form"
                    class="
                    @if(auth()->user()->hasRole('delivery'))
                    d-none
                    @endif
                    ">
                        ...
                        <div class="input-group mb-3" style="width: 22rem;">
                            <input type="number" class="form-control form-control-sm" placeholder="Ingrese un monto" name="monto" id="monto" value="{{ $monto ?? '' }}" step="0.01">
                            <button type="submit" class="btn btn-primary" >Guardar</button>
                        </div>
                    </form>
                    
                    <label class="d-block my-4">Turno: {{ $turno == 0 ? 'Mañana' : 'Tarde' }}</label>
                    
                    <div class="table-responsive
                    @if(!$monto && !(auth()->user()->hasRole('delivery') || auth()->user()->hasRole('admin')))
                    d-none
                    @endif
                    ">              
                        <table class="table table-bordered table-sm w-auto" id="cash-table">
                            ...
                            <tbody>
                                ...
                                <!--{{-- TOTAL ANTICIPOS GENERAL (Inicial + Pendientes) --}}
                                <tr>
                                    <td><strong>TOTAL ANTICIPOS</strong></td>
                                    <td align="right">
                                        <strong>{{ number_format(($total_anticipadas_iniciales + $total_anticipadas_pendientes), 2) }}</strong>
                                    </td>
                                </tr> -->
                                ...
                                @foreach ($totales_por_metodo as $nombre => $total)
                                <tr>
                                    <td>{{ $nombre }}</td>
                                ...
                                <tr>
                                  ...
                                  <tr>
                                    <td class="fw-bold" colspan="2">Efectivo = {{ number_format($ventaDirecta, 2) }} + {{ number_format($ventaAnticipada, 2) }} - {{ number_format($gastos, 2) }}</td>
                                </tr>
                                <tr>
...
<script>
    ...
    var saldoEfectivo = {{ $saldo ?? 0 }};
    var turno = {{ $turno ?? 0 }};
    var isDelivery = {{ auth()->user()->hasRole('delivery') ? 'true' : 'false' }};
        ...
        try{
            ...
            const crearEncabezado = (conector, fecha) => {
                ...
                return conector
                    ...
                    .EscribirTexto("Musas - Cierre de caja\n")
                    .EstablecerAlineacion(0)
                    .EscribirTexto(`Fecha: ${fecha}\n`)
                    .EscribirTexto(`Usuario: {{ auth()->user()->email }}\n`)
                    .EscribirTexto(`Sede: {{ auth()->user()->headquarter ?  auth()->user()->headquarter->nombre : '' }}\n`)
            ...
            const crearPie = (impresionTexto, efectivo) => {
                ...
                let real = parseFloat(document.getElementById('monto').value) || 0;
    ...
    $('#store-cash-close-form').on('submit', function(e) {
        ...
        // Delivery no puede guardar
        if (isDelivery) {
            ToastMessage.fire({ 
                icon: 'error', 
                text: 'Los usuarios delivery no pueden guardar cierres de caja.' 
            });
            return;
        }
        ...
        let monto = $('#monto').val();
        let fecha = $('#date').val();
        // No enviar turno en el request para usuarios normales

        $.ajax({
            url: "{{ route('payment.storeCashClose') }}",
            method: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                monto: monto,
    ...
    // Solo para delivery: cambio de turno
    if (isDelivery) {
    ...
    // Agregar el turno seleccionado al formulario solo para delivery
    $('#date-form').on('submit', function(e) {
        e.preventDefault();
        let fecha = $('#date').val();
        let url = "{{ route('payment.cashClose') }}";
        
        if (isDelivery) {
            let turno = $('#turno-select').val();
            window.location.href = url + '?date=' + fecha + '&turno=' + turno;
        } else {
            window.location.href = url + '?date=' + fecha;
        }
    });
    ...
    document.getElementById('btnPDF').addEventListener('click', function() {
        ...
        // Calcular el total del sistema (efectivo)
        const total = saldoEfectivo; // Esta variable ya está definida en tu código
    
        // Enviar los datos por POST a una ruta que genere el PDF
        fetch("{{ route('cashClose.pdf') }}", {
            method: "POST",
            headers: {
            ...
            body: JSON.stringify({
                date: fecha,
                monto: monto,
                efectivo: total, // Agregar el total del sistema
                tabla: tabla
            })
        })
