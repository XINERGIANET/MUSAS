                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $anticipada->fecha_entrega }}</td>
                                    <td>{{ $anticipada->headquarter->nombre ?? '-' }}</td>
                                    <td>
                                        @if($anticipada->voucher_type == 'Ticket')
                                            <a href="{{ route('sales.pdf', $anticipada) }}" target="_blank" class="btn btn-primary btn-sm btn-icon" title="Ver Ticket">
                                                <i class="bi bi-file-earmark-pdf"></i>
                                            </a>
                                        @endif
                                        @if($anticipada->voucher_type == 'Boleta' || $anticipada->voucher_type == 'Factura')
                                            <a href="{{ config('apisunat.url').'/documents/'.$anticipada->voucher_id.'/getPDF/ticket80mm/'.$anticipada->voucher_file }}" target="_blank" class="btn btn-primary btn-sm btn-icon" title="Ver Comprobante">
                                                Ticket
                                            </a>
                                            <a href="{{ config('apisunat.url').'/documents/'.$anticipada->voucher_id.'/getPDF/A4/'.$anticipada->voucher_file }}" target="_blank" class="btn btn-primary btn-sm btn-icon" title="Ver Comprobante">
                                                A4
                                            </a>
                                        @endif
                                        <button type="button"
...
<script>
    ...
    function agregarProductosImpresion(impresionTexto, productos) {
        ...
        const productosPanVarios = productos.filter(prod => parseInt(prod.id) === ID_PAN_VARIOS);
        const otrosProductos = productos.filter(prod => parseInt(prod.id) !== ID_PAN_VARIOS);
