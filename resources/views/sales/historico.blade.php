                ...
                <div class="card-body border-bottom">
                    <form action="" id="formFilter">
                        <div class="row d-flex">
                            ...
                            @php
                            $isAdmin = auth()->user()->hasRole('admin') || auth()->user()->hasRole('Xinergia');
                            $hq = auth()->user()->headquarter ? auth()->user()->headquarter->id : -1;
                            @endphp

                            <div class="col-md-3" @if(!$isAdmin) hidden @endif>
                                <div class="mb-3">
                                    <label class="form-label">Sede</label>
                                    <select class="form-select" id="headquarter_id" name="headquarter_id" @if(!$isAdmin) disabled @endif>
                                        <option value="">Todas las sedes</option>
                                        @foreach ($sedes as $sede)
                                        <option value="{{ $sede->id }}"
                                            {{ (request('headquarter_id') == $sede->id || (!$isAdmin && $hq == $sede->id)) ? 'selected' : '' }}>
                                            {{ $sede->nombre }}
                                        </option>
                                        @endforeach
                                    </select>
                            ...
                            {{-- Campo oculto para no-admins para que su sede se envíe de todos modos --}}
                            @if(!$isAdmin)
                            <input type="hidden" name="headquarter_id" value="{{ $hq }}">
                            @endif

                            <div class="col-md-3">
                                <label class="form-label">N° Comprobante</label>
                                <input type="text" id="numero_comprobante" name="numero_comprobante" class="form-control" value="{{ request('numero_comprobante') }}">
                            </div>
                            <div class="col-md-3 mb-3">
                                <label class="form-label">Cliente</label>
                                <input type="text" id="nombre_cliente" name="nombre_cliente" class="form-control" value="{{ request('nombre_cliente') }}">
                            </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="type_sale" class="form-select">
                                    ...
                                    <option value="0000" {{ request('type_sale') == '0000' ? 'selected' : '' }}>Directa</option>
                                    <option value="0001" {{ request('type_sale') == '0001' ? 'selected' : '' }}>Anticipada</option>
                                    <option value="0000-0001" {{ request('type_sale') == '0000-0001' ? 'selected' : '' }}>Directa y anticipada</option>
                                    <option value="0002-0003" {{ request('type_sale') == '0002-0003' ? 'selected' : '' }}>Delivery</option>
                                    <option value="restaurant" {{ request('type_sale') == 'restaurant' ? 'selected' : '' }}>Restaurante</option>
                                </select>
                            </div>
                            <div class="col-md-3">
                            ...
                            <div class="col d-flex align-items-end">
                                ...
                                <div class=" w-50s me-2">
                                    <a href="{{ route('sales.historico') }}" class="btn btn-warning w-100" id="btnLimpiar">Limpiar</a>
                                </div>
                            </div>
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($anticipada->type_sale == 0 && $anticipada->restaurant == 1)
                                        Restaurante
                                        @elseif($anticipada->type_sale == 0)
                                        Directa
                                        @elseif($anticipada->type_sale == 1)
                                        Anticipada
                                        @elseif($anticipada->type_sale == 2 || $anticipada->type_sale == 3)
                                        Delivery
                                        @else
                                        -
                                        @endif
                                    </td>
                                    <td>{{ $anticipada->cliente ?? $anticipada->client->nombre ?? "varios" }}</td>
                                    ...
                                    <td>
                                        ...
                                        <div class="payment-item mb-1">
                                            ...
                                            <span class="">S/ {{ number_format($payment->monto, 2) }}</span>
                                            <small>
                                                <a href="#"
                                    ...
                                    <td>{{ $anticipada->fecha_entrega ? $anticipada->fecha_entrega : "N/A" }}</td>
                                    ...
                                    <td>
                                        ...
                                        <button type="button"
                                            ...
                                            data-sale-id="{{ $anticipada->id }}"
                                            title="{{ $anticipada->estado == 1 ? 'Venta anulada' : 'Eliminar venta' }}"
                                            {{ $anticipada->estado == 1 ? 'disabled' : '' }}>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        buttons_detalle.forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (ruta_foto && ruta_foto !== '') {
                    link.href = '../storage/app/public/' + ruta_foto;
        ...
        // const agregar_pago = document.querySelector('#agregar-pago');
        // agregar_pago.addEventListener('click', function() {
        //     const sale_id = document.getElementById('modal-sale_id').value;
        ...
        //     $.ajax({
        //         url: "{{ route('payment.store') }}?sale_id=" + sale_id,
        //         method: 'GET',
        //         success: function(response) {
        //             let total = 0;
...
<script>
    ...
    $(document).on('click', '.btn-print-sale', function() {
        ...
        $.ajax({
            url: '{{ url('
            sales ') }}/' + saleId, // O usa "{{ url('sales') }}/" + saleId si tu ruta es resource
            method: 'GET',
    ...
    function crearDocumentoImpresion(conector, response, voucherTypeFormatted, productos, totalVenta, tipoComprobante, esAnticipada, esDelivery) {
        ...
        let impresionTexto = conector
            .EstablecerAlineacion(1)
            .EscribirTexto("Musas Pasteleria\n")
